...
<?php while($row = $result->fetch_assoc()): ?>
  <div class="col-md-4">
    <div class="card p-4 text-center border-primary border-2">
      ...
      <p class="price">Rp <?= number_format($row['harga_layanan'], 0, ',', '.'); ?> / kg</p>
      <p>
        <?php
          // deskripsi otomatis berdasarkan nama layanan
          if (strcasecmp($row['nama_layanan'], 'Cuci Kering') == 0) {
    echo 'Cuci cepat, bersih, wangi, dan kering maksimal setiap hari!';
} elseif (strcasecmp($row['nama_layanan'], 'Cuci Setrika') == 0) {
    echo 'Rapi, harum, dan nyaman dipakai – layanan lengkap kami.';
} elseif (strcasecmp($row['nama_layanan'], 'Setrika') == 0) {
    echo 'Setrika halus, rapi, dan siap pakai tanpa repot.';
} else {
    echo 'Layanan laundry terbaik dari Kinara Laundry.';
}

        ?>
      </p>
      <button class="btn btn-outline-primary btn-order mt-3"
        onclick="window.location.href='formulir.php?layanan=<?= urlencode($row['nama_layanan']); ?>'">
        Pesan Sekarang
      </button>
    </div>
  </div>
